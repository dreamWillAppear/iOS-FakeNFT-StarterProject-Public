...
final class RatingTableViewController: UIViewController {
    ...
    private func sortingButtonTapped() {
        ...
        let nameAction = UIAlertAction(title: "По имени", style: .default) { [weak self] _ in
            self?.changeSorting(sortingKey: .name) // Вызываем действие при нажатии
        }
        ...
        let cancelAction = UIAlertAction(title: "Закрыть", style: .cancel, handler: nil) //затемнить экран и кнопки кроме закрыть
...
extension RatingTableViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.setupCell(position: cellInfo.position, score: cellInfo.score, name: cellInfo.name, imageString: cellInfo.imageURLString)
